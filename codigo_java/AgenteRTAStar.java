...
public class AgenteRTAStar extends AbstractPlayer {
	...
	public AgenteRTAStar(StateObservation stateObs, ElapsedCpuTimer elapsedTimer){
        ...
        for (int i=0; i< immovable.length; i++){
            for (int j=0; j<immovable[i].size(); j++){
                ...
                if (obs.itype == 3 || obs.itype == 5) { // trampas y muros
                    mapaObstaculos[x][y] = 'm';
                }
                else if (obs.itype == 6){               // muros rojos
                    mapaObstaculos[x][y] = 'r';
                }
                else if (obs.itype == 7){               // muros azules
                    mapaObstaculos[x][y] = 'a';
                }
            }
        }
	...
	public Types.ACTIONS act(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
        ...
        int idxSegundo, segundaHeu, idxUnico, heuUnica, heuActual;
        ...
        if (!actual.posicion.equals(portal))
        {
            ...
            for (int i=0; i<sucesores.size(); i++){
                ...
                f_sucesores.add(heu_dinamicas.get(sucesor) + 1);
            ...
            // determinamos el segundo minimo de sucesores y actualizamos el valor heuristico del nodo actual
            // (regla de aprendizaje)
            heuActual = heu_dinamicas.get(actual);

            if (sucesores.size() > 1) {
                idxSegundo = f_sucesores_ordenado.get(1);
                segundaHeu = heu_dinamicas.get(sucesores.get(idxSegundo));
            
                if (segundaHeu +1 > heuActual)
                    heu_dinamicas.replace(actual, heuActual, segundaHeu +1);
            }
            else
            {
                // solo hay un sucesor
                idxUnico = f_sucesores_ordenado.get(0);
                heuUnica = heu_dinamicas.get(sucesores.get(idxUnico));
                if (heuUnica +1 > heuActual)
                    heu_dinamicas.replace(actual, heuActual, heuUnica +1);
            }

            // nos movemos al mejor vecino
            actual = sucesores.get(f_sucesores_ordenado.get(0));
            ...
            if (actual.posicion.equals(portal))
            {
                ...
                System.out.println("Runtime RTAStar (ms): " + tiempoTotalms);
